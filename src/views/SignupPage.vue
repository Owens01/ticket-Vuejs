...
function validate() {
  ...
  if (password.value !== confirmPassword.value) e.confirmPassword = 'Passwords do not match';
  return e;
}
...
function handleSubmit() {
  ...
  const result = auth.signup(email.value, password.value, confirmPassword.value);
  if (result.success) {
    toast.showToast('Account created successfully!', 'success');
    router.push({ name: 'Dashboard' });
  } else {
    toast.showToast(result.error ?? 'Signup failed', 'error');
  }
}
</script>
